...
//   const { data: statusData, error, isLoading } = useGetApplicationStatusQuery();
...
//   console.log(statusData?.success); // Safe now
...
const ApplicantStatusPage = ({ status: initialStatus }: StatusPageProps) => {
  ...
  // 2. Get application ID from status response
  const applicationId = statusResponse?.data?.id;

  // 3. Fetch full details if we have an ID
  const {
  ...
  // Use either passed status or fetched status
...
const getTimelineStages = (status: string) => ({
  ...
  if (!applicationData) return (
    <p className="text-center text-gray-500 mt-40 text-xl">
